...
class DAO
{
	...
	{	// préparation de la requete de recherche
		$txt_req = "Select * from mrbs_entry, mrbs_entry_digicode, mrbs_room ";
		$txt_req = $txt_req . " where mrbs_entry.room_id = mrbs_room.id ";
		$txt_req = $txt_req . " and mrbs_entry.id = mrbs_entry_digicode.id";
	...
	{	// préparation de la requete de recherche
		...
		{	// création d'un objet Salle
			$unId = $uneLigne->id;
			$unRoomName = $uneLigne->room_name;
			$unCapacity = $uneLigne->capacity;
			$unAreaName = $uneLigne->area_name;
			$unAeraAdminEmail = $uneLigne->area_admin_email;
